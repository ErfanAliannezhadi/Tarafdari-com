...
from statuses.forms import StatusCreationForm, StatusCommentForm
...
class UserProfileView(ProfileContextMixin, LoginRequiredMixin, UnBlockedRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['status_creation_form'] = StatusCreationForm()
        context['status_create_comment_form'] = StatusCommentForm()
        context['statuses'] = StatusModel.objects.filter(to_user=context['user'], is_private=False).annotate(
            is_liked=Case(
...
class UserFollowingsListView(ProfileContextMixin, LoginRequiredMixin, UnBlockedRequiredMixin, ListView):
    paginate_by = 20
    template_name = 'accounts/user_followers_followings.html'
    context_object_name = 'f_users'

    def get_queryset(self):
        return UserModel.objects.get(pk=self.kwargs['pk']).followings.all()


class UserFollowersListView(ProfileContextMixin, LoginRequiredMixin, UnBlockedRequiredMixin, ListView):
    paginate_by = 20
    template_name = 'accounts/user_followers_followings.html'
    context_object_name = 'f_users'

    def get_queryset(self):
        return UserModel.objects.get(pk=self.kwargs['pk']).followers.all()
